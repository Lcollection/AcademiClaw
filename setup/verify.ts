...
export async function run(_args: string[]): Promise<void> {
  ...
  if (mgr === 'launchd') {
    try {
      ...
      if (output.includes('com.academiclaw')) {
        // Check if it has a PID (actually running)
        const line = output.split('\n').find((l) => l.includes('com.academiclaw'));
        if (line) {
    ...
    try {
      execSync(`${prefix} is-active academiclaw`, { stdio: 'ignore' });
      service = 'running';
    } catch {
      try {
        ...
        if (output.includes('academiclaw')) {
          service = 'stopped';
        }
      } catch {
  ...
  if (
    fs.existsSync(
      path.join(homeDir, '.config', 'academiclaw', 'mount-allowlist.json'),
    )
  ) {
