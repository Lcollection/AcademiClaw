...
<dict>
    ...
    <string>${projectRoot}/logs/academiclaw.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/academiclaw.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.academiclaw');
  } catch {
...
/**
 * Kill any orphaned academiclaw node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned academiclaw processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/academiclaw.service';
    ...
    unitPath = path.join(unitDir, 'academiclaw.service');
...
StandardOutput=append:${projectRoot}/logs/academiclaw.log
StandardError=append:${projectRoot}/logs/academiclaw.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned academiclaw processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable academiclaw`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start academiclaw`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active academiclaw`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-academiclaw.sh');
  const pidFile = path.join(projectRoot, 'academiclaw.pid');

  const lines = [
    '#!/bin/bash',
    '# start-academiclaw.sh — Start AcademiClaw without systemd',
    ...
    '    echo "Stopping existing AcademiClaw (PID $OLD_PID)..."',
    '    kill "$OLD_PID" 2>/dev/null || true',
    '    sleep 2',
    '  fi',
    'fi',
    '',
    'echo "Starting AcademiClaw..."',
    `nohup ${JSON.stringify(nodePath)} ${JSON.stringify(projectRoot + '/dist/index.js')} \\`,
    `  >> ${JSON.stringify(projectRoot + '/logs/academiclaw.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/academiclaw.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "AcademiClaw started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/academiclaw.log"`,
  ];
